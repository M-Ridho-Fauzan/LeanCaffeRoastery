...
import { BotMessageSquare, Clock, Headphones, MessageCircle, MessageSquare, Phone, Send, User } from 'lucide-react';
...
export default function ChatbotField() {
    // Tinggi minimum untuk konten chat agar tidak terlalu kecil di ponsel yang sangat pendek
    const CONTENT_MIN_HEIGHT = 'calc(85vh - 120px)';
        ...
        <div className="">
            <Popover>
                <PopoverTrigger asChild>
                    <Button className="fixed right-4 bottom-4 h-fit w-fit rounded-full bg-[#D9D9D9] py-3 text-[##212544] shadow-lg hover:bg-[#3d4d75] hover:text-white md:right-14 md:bottom-14">
                        <MessageCircle className="size-10 md:size-14" />
                    </Button>
                </PopoverTrigger>

                <PopoverContent
                    className="right-4 w-full max-w-[calc(100vw-32px)] overflow-hidden rounded-2xl bg-[#A8A9AA] p-0 sm:max-w-[400px] md:right-10 md:max-w-[500px] md:rounded-4xl"
                ...
                >
                    <Tabs defaultValue="ai">
                        ...
                        <header className="flex flex-col items-center rounded-b-[28px] bg-[#212544] px-4 pt-6 pb-4 text-white">
                            {/* Logo + Title */}
                            <div className="mb-2.5 flex flex-col items-center text-center sm:flex-row">
                                <AppLogoIcon className="mr-3 size-16 fill-white sm:size-24" />
                                <h1 className="text-sm font-extralight sm:text-lg">Lean Coffee Roastery Support</h1>
                            </div>

                            {/* Tabs */}
                            {/* TabsList: Mengurangi margin dan gap di mobile, mengembalikan di desktop */}
                            <TabsList className="mx-2 -mb-14 flex w-[90%] gap-2 rounded-xl bg-transparent p-1 py-10 *:border *:bg-[#D9D9D9] *:py-4 *:text-[#212544] *:shadow-2xl data-[state=active]:mx-1 *:data-[state=active]:border *:data-[state=active]:border-[#212544] sm:mx-10 sm:w-auto sm:gap-16">
                                <TabsTrigger
                                    value="ai"
                                    className="flex flex-1 items-center justify-center gap-1 rounded-lg text-xs data-[state=active]:bg-[#D9D9D9] data-[state=active]:text-[#212544] sm:text-sm"
                                >
                                ...
                                <TabsTrigger
                                    value="live"
                                    className="flex flex-1 items-center justify-center gap-1 rounded-lg text-xs *:fill-[#212544] data-[state=active]:bg-[#D9D9D9] data-[state=active]:text-[#212544] sm:text-sm"
                                >
                        ...
                        <TabsContent value="ai" className="m-0 mt-9 p-0">
                            <ChatbotUI minHeight={CONTENT_MIN_HEIGHT} />
                        </TabsContent>
                        <TabsContent value="live" className="m-0 mt-9 p-0">
                            <LiveSupportUI minHeight={CONTENT_MIN_HEIGHT} />
                        </TabsContent>
                    </Tabs>
                </PopoverContent>
            </Popover>
...
// Menambahkan interface untuk menerima prop minHeight
interface ChatbotUIProps {
    minHeight: string;
}

// --- Chatbot UI Component ---
const ChatbotUI: React.FC<ChatbotUIProps> = ({ minHeight }) => {
    const chatMessages = [
        {
            id: 1,
            type: 'bot' as const,
    ...
    return (
        // Tinggi disesuaikan menggunakan prop style berbasis viewport height (vh)
        <div className={'flex w-full flex-col overflow-hidden shadow-lg'} style={{ maxHeight: minHeight, minHeight: '300px' }}>
            {/* Chat Area */}
                    ...
                    <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {message.type === 'bot' && (
                            <div className="flex items-start">
                                <div className="mr-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-200 p-1 text-[#212544]">
                                    <MessageSquare size={16} />
                                </div>
                                {/* max-w-xs agar tidak memakan terlalu banyak ruang di layar kecil */}
                                <div className="max-w-[75%] rounded-xl rounded-tl-none bg-gray-100 p-3 shadow-sm sm:max-w-xs">
                        ...
                        {message.type === 'user' && (
                            <div className="flex items-start">
                                {/* max-w-xs agar tidak memakan terlalu banyak ruang di layar kecil */}
                                <div className="max-w-[75%] rounded-xl rounded-br-none bg-[#212544] p-3 text-white shadow-sm sm:max-w-xs">
                                    <p className="text-sm">{message.text}</p>
                                    <p className="mt-1 text-left text-[10px] opacity-70">{message.time}</p>
                                </div>
                                <div className="ml-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-200 p-1 text-[#212544]">
                                    <User size={16} />
                                </div>
                            </div>
                        )}
            ...
            {/* Input Footer */}
            {/* Mengganti py-14 yang terlalu besar menjadi p-3 yang ringkas */}
            <footer className="flex items-center gap-3 border-t bg-[#212544] p-3">
                <Input
                    type="text"
...
const LiveSupportUI: React.FC<ChatbotUIProps> = ({ minHeight }) => {
    return (
        // Tinggi disesuaikan menggunakan prop style berbasis viewport height (vh)
        <div className="flex w-full flex-col overflow-hidden shadow-lg" style={{ maxHeight: minHeight, minHeight: '300px' }}>
            {/* Main Content */}
            {/* Padding disesuaikan lebih kecil di mobile */}
            <div className="flex flex-1 flex-col items-center justify-start space-y-4 overflow-y-auto p-4 sm:space-y-6 sm:p-8">
                <div className="flex items-center justify-center rounded-full text-[#212544]">
                    <Headphones className="size-16 sm:size-24" />
                </div>
                <h2 className="text-xl font-bold text-[#212544] sm:text-2xl">Live Customer Support</h2>
                <p className="text-center text-xs text-[#212544] sm:text-sm">
                    Connect with our customer service team
                    <br className="hidden sm:block" />
                    for personalized assistance
                </p>

                {/* Card Container: Menggunakan w-full, max-w-xs untuk responsifitas */}
                <div className="w-full max-w-xs space-y-4">
                    {/* Support Hours Card */}
                    <div className="w-full rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                        <div className="flex items-center">
                            <div className="mr-4 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-100 text-[#212544]">
                                <Clock size={20} />
                            </div>
                            <div>
                    ...
                    <Button className="mt-2 w-full rounded-lg bg-[#212544] py-3 text-white hover:bg-[#3d4d75]">Start Live Chat</Button>
...
// Icon Call Center (Dibiarkan sama, hanya memperbaiki penamaan prop di JSX)
